class ServicesController < ApplicationController
  ...
  def index
    ...
    begin
      if request.post? || session[:user_login_service] != "twitter"
        ...
        ActionMailer::Base.mail(:from => ActionMailer::Base.smtp_settings[:user_name], :to => @email, :subject => "ListsofLists Website Usage Link", :body => @message_body, content_type: "text/html").deliver
  ...
  def create
    ...
    if omniauth
      ...
      if service_route == 'facebook' || service_route == 'google_oauth2'
        omniauth['info']['email'] ? email =  omniauth['info']['email'] : email = ''
        omniauth['info']['name'] ? name =  omniauth['info']['name'] : name = ''
        omniauth['uid'] ?  uid =  omniauth['uid'] : uid = ''
        omniauth['provider'] ? provider =  omniauth['provider'] : provider = ''
      elsif service_route == 'twitter'
        session[:user_login_service] = "twitter"
        email = omniauth['info']['nickname'] + "@twitter_2013.com" # Create a dummy email for storing
        omniauth['info']['name'] ? name =  omniauth['info']['name'] : name = ''
        omniauth['uid'] ?  uid =  omniauth['uid'] : uid = ''
        omniauth['provider'] ? provider =  omniauth['provider'] : provider = ''
      else
